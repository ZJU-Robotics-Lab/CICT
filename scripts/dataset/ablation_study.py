...
parser.add_argument('--dataset_name', type=str, default="kitti-abs-img-01", help='name of the dataset')
...
description = 'kitti-abs-no-v0-01'
...
trajectory_dataset = TrajectoryDatasetASImg(param, 'train')#7
dataloader = DataLoader(trajectory_dataset, batch_size=opt.batch_size, shuffle=False, num_workers=opt.n_cpu)

eval_trajectory_dataset = TrajectoryDatasetASImg(param, 'eval')#2
...
def eval_metric(step):
    ...
    batch['img'] = batch['img'].expand(len(t),10,3,opt.height, opt.width)
