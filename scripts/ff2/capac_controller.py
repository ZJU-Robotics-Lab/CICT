...
class CapacController(object):
    ...
    def run_step(self, trajectory, index, state0):
        ...
        #self.world.debug.draw_point(localtion, size=0.2, color=carla.Color(255,0,0), life_time=0.2)
        # throttle, brake = 1, 0
        throttle += 0.5#15
        ...
        vy = vehicle_v.y
        vz = vehicle_v.z
        global_vel = np.sqrt(vx**2+vy**2+vz**2)
        print(abs(global_vel), abs(target_state.v))
        if abs(global_vel) < 1.8 and abs(target_state.v) < 1.8:
            throttle = 0.
            brake = 1.
            steer = 0.
