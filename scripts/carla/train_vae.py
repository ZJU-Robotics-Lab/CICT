...
from learning.path_model import ModelVAE
...
parser.add_argument('--dataset_name', type=str, default="vae-01", help='name of the dataset')
...
parser.add_argument('--gamma3', type=float, default=1.0, help='KLD loss')
...
def draw_traj(step):
    ...
    output, mu, logvar = model(batch['img'], t, batch['v_0'])
...
def eval_error(total_step):
    ...
    for i in range(100):
        ...
        output, mu, logvar = model(batch['img'], batch['t'], batch['v_0'])
...
for i, batch in enumerate(train_loader):
    ...
    loss = loss_xy + opt.gamma*loss_vxy + opt.gamma2*loss_axy + opt.gamma3*KLD
