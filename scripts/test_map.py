...
def draw_route(agent, destination, origin_map):
    ...
    for i in range(len(route_trace)):
        ...
        route_trace_xs.append(x)
        route_trace_ys.append(y)
        
    draw = ImageDraw.Draw(origin_map)
    for i in range(len(route_trace_xs)-1):
        draw.line((route_trace_xs[i], route_trace_ys[i], route_trace_xs[i+1], route_trace_ys[i+1]), 'red', width=12)
